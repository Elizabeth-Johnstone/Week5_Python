...
# Use a for-in loop to iterate over the `numbers` list, and if a number is even, add  it to the `even_numbers` array.
even_numbers = []
for nums in numbers:
    if nums % 2 == 0:
...
# Determine the letter grade of the given variable 'score'. If the variable is a 90 or above, print an 'A', between 80 and 89, print a 'B', between 70 and 79, 'C', between 60 and 69, 'D', and anything below 60 should print an 'F'.
if score < 60:
    print('F')
elif score < 70:
    print('D')
...
# Problem 12
# Research to find the Python method that allows you to add an element to the end of the list (similar to push() in JavaScript), then add a name to the end of the `friends` array.
friends.append("Ezekiel")
